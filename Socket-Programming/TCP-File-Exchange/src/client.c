...
int main (int argc, char* argv[])
{
	...
	while (1)
	{
		...
		while (1)
		{
			...
			// Create BUFFER
			fprintfSwitchable(NULL, 0, "[+client] Acceptable Buffer-Size is Sent Client.\n"); 
			...
			fprintfSwitchable(NULL, 0, "[+client] Buffer Sent Successfully.\n"); 
			
			// Recv ACK From Server
			if (read(sockfd, &ACKFLAG, sizeof(int)) == -1)	
				fprintfSwitchable(NULL, 1, "[-client] Error While Recieving ACK!\n"); 
			
			fprintfSwitchable(NULL, 0, "[+client] Persist ACK is Recieved From Server.\n");  

			// In Session Statistics	
			remainMem      -= BUFFER_USED;
			fileMemory_ACK += BUFFER_USED;
			fprintfSwitchable(NULL, 0, "[+client] ACK Memory is %lu :: Buffer-Used is %lu :: Remaining Memory is: %lu\n", 
