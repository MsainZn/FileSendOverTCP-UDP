/*
	...
	(1) make all
		if no make was provided then gcc client.c utilityFunctionsMHZ.h -o client
	(2) ./bin/client FILENAME SERVER_ADDR PORT_NUMBER BUFFER_SIZE
		Example: ./client mainFile.log 172.17.34.89 5500 1024
	(3) ./bin/client FILENAME SERVER_ADDR PORT_NUMBER 
...
int main (int argc, char* argv[])
{
	...
	if (argc < 4) 
	{	fprintf(stderr, "Please use the given format: ./client FILENAME SERVER_ADDR PORT_NUMBER BUFFER_SIZE\n\
 		    Example: ./client mainlog.log 172.17.34.89 5500 1024\n");
		exit(0);
	}

	// Input Parameters  
	char* logFileName		 = argv[1]; 			    //"mainFile.log";
	//struct hostent* serverIP = gethostbyname(argv[2]);	// Server IP Address such as: "192.168.1.1"; Also char* == struct hostent*
	...
	while (1)
	{
		...
		if (write(sockfd, bufferPtr, BUFFER_MAX) == -1)
			CheckError("[-client] Error While Writing from buffer!\n");
		...
		if (read(sockfd, bufferPtr, BUFFER_MAX) == -1)
			CheckError("[-client] Error While Read from buffer!\n");
		else
			printf("Server >> %s", bufferPtr);		
	}
